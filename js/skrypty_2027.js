$(document).ready(function () {
    ...
    $('.d_roku_1').find('.nr_dnia p').html('29');
    $('.d_roku_2').find('.nr_dnia p').html('30');
    // $('.d_roku_3').find('.nr_dnia p').html('29');
    // $('.d_roku_4').find('.nr_dnia p').html('30');

    // W 2027
    $('.d_roku_2').find('.dane').prepend('<p class="swieto">św Andrzeja ap (ś)</p>');
    $('.d_roku_2').find('.dane .sigla').html('Rz 10,9-18; Ps 19; Mt 4,18-22')
    $('.d_roku_2').find('.dane .prawe .g_czyt').html('1 Kor 1,18-2,5');


    // II Niedziela Wielkanocna - Miłosierdzia Bożego (u)
    // $('.d_roku_135').find('.dane .prawe').prepend('<p class="oznaczenie">II</p>');

    // Zwiastowanie Pańskie przeniesione - kolidowało z wlk Czw w 2027
    $('.d_roku_128 .dane').prepend('<p class="swieto">Zwiastowanie Pańskie (u)</p>');
    $('.d_roku_128').find('.dane .sigla').text('Iz 7,10-14;8,10c; Ps 40; Hbr 10,4-10; Łk 1,26-38');
    $('.d_roku_128').find('.dane .prawe .g_czyt').text('gcz: 1 Krn 17,1-15');
    $('.d_roku_128').find('.dane .wspomnienie').prepend('*');
    ...
    // Jeśli Boże Narodzenie wypadnie w sobotę, to nałożą się kolory
    $('.d_roku_' + narodzeniePanskie2).find('.niedziela').css('background-color', 'transparent');
    ...
    // Wielki Czwartek
    $('.d_roku_' + (wielkanoc - 3)).find('.dane .swieto').remove(); // Jeśli wypada jakieś święto
    ...
    // Zwiastowanie Pańskie w 2027 wypadło w Wlk. Czwartek - do usunięcia
    $('.d_roku_' + (wielkanoc - 3)).find('.dane .obchod').html('<p>Zwiastowanie Pańskie (u)</p>');
    $('.d_roku_' + (wielkanoc - 3)).find('.dane .prawe .g_czyt').html('gcz: Lm 2,10-22');
    // Wielki Piątek
